...
def fixed_feature_config(prefix, args):
    return {
        f'{prefix}_use_normalized_gap': getattr(args, f'{prefix}_use_normalized_gap'),
        f'{prefix}_use_recent_compactness': getattr(args, f'{prefix}_use_recent_compactness'),
        f'{prefix}_use_recency_score': getattr(args, f'{prefix}_use_recency_score'),
    }


def main():
    parser = argparse.ArgumentParser(description='Run SASRec ablations with fixed long/short continuous features and no A2/B1b/B2 stages.')
    ...
    parser.add_argument('--output_csv', default='experiment_master_fixed_features.csv')
    ...
    parser.add_argument('--long_use_normalized_gap', default=True, type=str2tristate)
    parser.add_argument('--long_use_recent_compactness', default=True, type=str2tristate)
    parser.add_argument('--long_use_recency_score', default=True, type=str2tristate)
    parser.add_argument('--cnn_use_normalized_gap', default=True, type=str2tristate)
    parser.add_argument('--cnn_use_recent_compactness', default=True, type=str2tristate)
    parser.add_argument('--cnn_use_recency_score', default=True, type=str2tristate)
    parser.add_argument(
        '--b1a_config',
        action='append',
        default=[],
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    from run_experiments import make_failed_row, stable_config_id
    ...
    # Stage B1a: structure search with fixed long/short continuous features
    b1a_configs = []
    fixed_long_cfg = fixed_feature_config('long', args)
    fixed_cnn_cfg = fixed_feature_config('cnn', args)
    if args.b1a_config:
        for spec in args.b1a_config:
            cfg_name, cfg = parse_b1a_config_spec(spec)
            merged_cfg = dict(best_a1_cfg)
            merged_cfg.update(cfg)
            merged_cfg.update(fixed_long_cfg)
            merged_cfg.update(fixed_cnn_cfg)
        ...
        for n_blocks, ksz, rw, ghu in itertools.product(
            ...
            cfg.update(fixed_long_cfg)
            cfg.update(fixed_cnn_cfg)
    ...
    print('\nBest B1a config:')
    print(f"  config_name = {best_b1a['config_name']}")
    print(f"  best_val_ndcg_mean = {best_b1a['best_val_ndcg_mean']}")
    print(f"  best_val_hr_mean = {best_b1a['best_val_hr_mean']}")
    print(f"  best_val_ndcg_std = {best_b1a.get('best_val_ndcg_std')}")
    print(f"  best_val_hr_std = {best_b1a.get('best_val_hr_std')}")
